...
def test_compute_molar_mass():
    ...
    # Create the periodic table first
    periodic_table = make_periodic_table()

    water_formula = [["H", 2], ["O", 1]] 
    expected_water_mass = 2*1.00794 + 15.9994
    assert abs(compute_molar_mass(water_formula, periodic_table) - expected_water_mass) < 1e-6

    wine_formula = [["C", 2], ["H", 5], ["O", 1]] 
    expected_wine_mass = 2*12.0107 + 5*1.00794 + 15.9994
    assert abs(compute_molar_mass(wine_formula, periodic_table) - expected_wine_mass) < 1e-6
